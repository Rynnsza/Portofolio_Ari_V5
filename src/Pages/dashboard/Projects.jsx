  ...
  <div className="relative">
    ...
    <div className="relative bg-white/5 border border-white/12 rounded-2xl p-4 flex flex-col gap-3">
      ...
      <div className="flex gap-1.5 mt-1">
        {[1,2,3].map(i => <div key={i} className="h-5 w-16 bg-white/5 animate-pulse rounded-full" />)}
      </div>
    </div>
  </div>
);
  ...
  return (
    <Card>
      <div className="p-4 flex flex-col h-full">
        ...
        {project.description && (
          <p className="text-gray-400 text-xs mb-3 line-clamp-2 leading-relaxed">{project.description}</p>
        )}
        {project.tech_stack?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-3">
            {project.tech_stack.map((t) => (
              <span key={t} className="px-2 py-0.5 rounded-full bg-indigo-500/15 border border-indigo-500/25 text-indigo-300 text-xs">{t}</span>
            ))}
          </div>
        )}
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form, file); }} className="p-5 sm:p-6 space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="sm:col-span-2">
          <InputField label="Project Title" value={form.title} onChange={set("title")} placeholder="e.g. My Portfolio Website" required />
        </div>
        ...
        <InputField label="Tech Stack (comma separated)" value={form.tech_stack} onChange={set("tech_stack")} placeholder="e.g. React, Tailwind, Supabase" />
...
const fetchProjects = async () => {
  ...
  console.log('Fetch error:', error) // ← tambah ini
  console.log('Fetch data:', data)   // ← tambah ini
  ...
  const handleCreate = async (form, file) => {
    ...
    try {
      ...
      const { error } = await supabase.from("projects").insert({
        ...
        github: form.github ? (form.github.startsWith("http") ? form.github : `https://${form.github}`) : "",
      });
