...
const ProjectDetails = () => {
  ...
  useEffect(() => {
    ...
    const getProject = async () => {
      setLoading(true);
      try {
        // 1. Coba cari di localStorage dulu
        const storedProjects = JSON.parse(localStorage.getItem("projects")) || [];
        const cachedProject = storedProjects.find((p) => toSlug(p.title || p.Title) === slug);

        if (cachedProject) {
          setProject({
            ...cachedProject,
            title: cachedProject.title || cachedProject.Title,
            description: cachedProject.description || cachedProject.Description,
            img: cachedProject.img || cachedProject.Img,
            link: cachedProject.link || cachedProject.Link,
            github: cachedProject.github || cachedProject.Github,
            tech_stack: cachedProject.tech_stack || cachedProject.TechStack || [],
            features: cachedProject.features || cachedProject.Features || [],
          });
          setLoading(false);
        }

        // 2. Selalu fetch dari Supabase untuk memastikan data terbaru atau jika tidak ada di cache
        ...
        if (!error && data) {
          const foundProject = data.find((p) => toSlug(p.title) === slug);
          if (foundProject) {
  ...
  return (
    <>
      <Helmet>
        <title>{project.title} — Ahmad Ambari</title>
        <meta
          name="description"
          content={
            project.description
              ? project.description.slice(0, 155)
              : `Project ${project.title} oleh Ahmad Ambari — Frontend Web Developer.`
          }
        />
        ...
        <meta
          property="og:title"
          content={`${project.title} — Ahmad Ambari`}
        />
        <meta
